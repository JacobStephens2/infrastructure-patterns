...
records that matter more than web: MX, SPF, DKIM selectors, DMARC. The goal was to
bring all of it under Terraform so every later change is a reviewable diff.

There are two ways to put existing infrastructure under code. **Recreate:** author
a desired-state list of records and let Terraform create them, deleting whatever
doesn't match. **Import:** pull the records that already exist into Terraform state
and write configuration that matches them, changing nothing.
...
- **Enumerate every record from the provider API and emit one import block per
  record** (a small generator script, re-runnable). Let Terraform generate the
...
- **Drive to a zero-change plan.** Done correctly, `terraform plan` reports *N to
  import, 0 to add, 0 to change, 0 to destroy*. That zero is the proof the code
  matches what is actually serving traffic.
...
- **Guard the create path off until parity exists.** A feature flag defaulting to
  "manage nothing" keeps an accidental apply from creating duplicates of records
  that are already live.

## Consequences

- **Zero downtime and no drift window.** Nothing is deleted or recreated; the
  records that serve traffic are the records Terraform adopts. This is the whole
...
- **Provider quirks surface at import, not later.** The generator emitted both
  ...
  rejects as mutually exclusive; the fix was a deterministic post-process. Importing
  forces these to the surface immediately instead of on a future edit.
- **The record list stops being authored and starts being derived.** Re-running the
  generator is also a drift audit: anything it surfaces that the config doesn't have
  was changed outside Terraform. That inverts the usual "config is truth" stance to
...
desired state directly and let Terraform build it, with no import dance. The same
holds if a record set is small enough to recreate inside a planned maintenance
window where a brief inconsistency is acceptable. The import-first rule is
specifically for adopting infrastructure that is already serving traffic - which is
most of the interesting cases, and the one where recreating quietly breaks things.
